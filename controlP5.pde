...
void gui() {
  ...
  // reset controls
  controlP5.addButton("resetView",0,40,285,65,40).setLabel("Reset Cam &\nShape Color");
  controlP5.addButton("resetLights",0,125,285,65,40).setLabel("Reset Lights");
  controlP5.addButton("reset",0,width-106,585,60,45).setLabel("      Reset\nEVERYTHING");
 ...
 controlP5.addTextlabel("lblBasic")
  ...
  for(int i=301; i<numForLoop+301; i++) {
    if (numToName(i) != "None") { shaderList.addItem(numToName(i),i); }
  }
...
void mouseWheel(int delta) {
  if      (delta > 0) { if (zoom > 20) { controlP5.getController("zoom").setValue(zoom - 10); } else { controlP5.getController("zoom").setValue(zoom - 1); } }
  else if (delta < 0) { if (zoom >= 20) { controlP5.getController("zoom").setValue(zoom + 10); } else { controlP5.getController("zoom").setValue(zoom + 1); } }
}
...
void onMouseOver() {
 ...
 if(mouseOverList.contains("myShapeList")) { g3.bringToFront(); shaderList.close(); modifyList.close(); }
 if(mouseOverList.contains("myModifyList")) { g3.bringToFront(); shaderList.close(); shapeList.close(); }
 if(mouseOverList.contains("myShaderList") && !mouseOverList.contains("myShapeList")) { g4.bringToFront(); shapeList.close(); modifyList.close(); }
...
void setShapeParameters(int shapeNum) {
 controlP5.getController("create0").setLabel("");controlP5.getController("create1").setLabel("");controlP5.getController("create2").setLabel("");controlP5.getController("create3").setLabel("");

 switch(shapeNum) {
   case 0 : controlP5.getController("create0").setLabel("Depth");controlP5.getController("create1").setLabel("Height");controlP5.getController("create2").setLabel("Width");break;
   case 1 : controlP5.getController("create0").setLabel("Radius");controlP5.getController("create1").setLabel("Height");controlP5.getController("create2").setLabel("Facets");controlP5.getController("create3").setLabel("Steps");break;
   case 2 : controlP5.getController("create0").setLabel("Edge");break;
   case 3 : controlP5.getController("create0").setLabel("Radius");controlP5.getController("create1").setLabel("Level");break;
   case 4 : controlP5.getController("create0").setLabel("Radius");controlP5.getController("create1").setLabel("UFacets");controlP5.getController("create2").setLabel("VFacets");break;
   case 5 : controlP5.getController("create0").setLabel("Radius");controlP5.getController("create1").setLabel("Height");controlP5.getController("create2").setLabel("Facets");controlP5.getController("create3").setLabel("Steps");break;
   case 6 : controlP5.getController("create0").setLabel("Edge");break;
   case 7 : controlP5.getController("create0").setLabel("Edge");break;
   case 8 : controlP5.getController("create0").setLabel("Edge");break;
   case 9 : controlP5.getController("create0").setLabel("Radius1");controlP5.getController("create1").setLabel("Radius2");controlP5.getController("create2").setLabel("TubeFacets");controlP5.getController("create3").setLabel("TorusFacets");break;
   case 10 : controlP5.getController("create0").setLabel("Width");controlP5.getController("create1").setLabel("Height");controlP5.getController("create2").setLabel("Disturb");break;
   case 11 : controlP5.getController("create0").setLabel("NB POINT\nx10000");controlP5.getController("create1").setLabel("Width");controlP5.getController("create2").setLabel("Height");controlP5.getController("create3").setLabel("Depth");break;
...
void setShaderParameters(int shaderNum) {
 ((Slider)controlP5.getController("param0")).setLabel("").setRange(0.0f, 1.0f).setValue(0.0f).setDecimalPrecision(2);((Slider)controlP5.getController("param1")).setLabel("").setRange(0.0f, 1.0f).setValue(0.0f).setDecimalPrecision(2);
 ((Slider)controlP5.getController("param2")).setLabel("").setRange(0.0f, 1.0f).setValue(0.0f).setDecimalPrecision(2);((Slider)controlP5.getController("param3")).setLabel("").setRange(0.0f, 1.0f).setValue(0.0f).setDecimalPrecision(2);

 switch(shaderNum) {
   case 301 : ((Slider)controlP5.getController("param0")).setLabel("shinyness").setRange(0.0f,1.0f).setValue(0.25f);break;
   case 302 : ((Slider)controlP5.getController("param0")).setLabel("Index Refraction").setRange(0.0f,5.0f).setValue(1.5f);((Slider)controlP5.getController("param1")).setLabel("absorptionDistance").setRange(0.0f,10.0f).setValue(5.0f);break;
   case 305 : ((Slider)controlP5.getController("param0")).setLabel("power").setRange(0,500).setValue(50).setDecimalPrecision(0);break;
   case 307 : ((Slider)controlP5.getController("param0")).setLabel("roughnessX").setRange(0.0f,1.0f).setValue(1.0f);((Slider)controlP5.getController("param1")).setLabel("roughnessY").setRange(0.0f,1.0f).setValue(1.0f);break;
   case 308 : ((Slider)controlP5.getController("param0")).setLabel("width").setRange(0.0f,1.0f).setValue(1.0f);break;
   default:break;
  }
}
...
void save() {
  if (saveOn) {
    ...
    if (saveOpenGL) { print(" | OpenGL Current View"); }
    if (saveGui) { print(" | Gui"); }
    if (saveSunflow) { print(" | Render Sunflow"); }
    if (saveMask) { print(" | Save Sunflow Mask"); }
    print("\n");
  }
}
...
void controlEvent(ControlEvent theEvent) {
  if (theEvent.isController()) {
    ...
    if (theEvent.getController().getName() == "myShaderList") {
      shader = int(theEvent.getController().getValue());
      controlP5.getController("lblCurrentShader").setValueLabel("SHADER : " + numToName(shader));
    }
    ...
    for (int i=0; i<5; i++) {
      ...
      if (theEvent.getController().getName().endsWith("v" + i)) {
        ...
        for (int j=0; j<5; j++) {
          if (i==j) { m.values[j] = theEvent.getValue(); }
        }
        createHemesh();
      }
...
void keyPressed() {
  ...
  if (key == 'r') { if (autoRotate == false)  { controlP5.getController("autoRotate").setValue(1);  } else { controlP5.getController("autoRotate").setValue(0);  } }
  if (key == 't') { if (translationOn == false) { controlP5.getController("translationOn").setValue(1); } else { controlP5.getController("translationOn").setValue(0); } }
  if (key == 'y') { if (rotationOn == false)    { controlP5.getController("rotationOn").setValue(1);    } else { controlP5.getController("rotationOn").setValue(0);    } }

  // toggle the controlP5 gui
  if (key == '5') { drawControlP5 = !drawControlP5; }

  // set X & Y speed of translation & rotation to zero
  if (key == '0') { controlP5.getController("changeSpeedX").setValue(0); controlP5.getController("changeSpeedY").setValue(0); }

  // toggle sunflow manually
  if (key == 's') { if (saveSunflow) { controlP5.getController("saveSunflow").setValue(0); } else { controlP5.getController("saveSunflow").setValue(1); } }
